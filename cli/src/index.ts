...
const SKIP_FILES = new Set(['pnpm-lock.yaml', 'rust-workers/pnpm-lock.yaml', 'CLAUDE.local.md', '.claude/settings.local.json'])
...
function walk(dir: string, base: string): string[] {
  ...
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (SKIP_FILES.has(rel) || rel.startsWith(path.join('apps'))) continue
...
function replace(s: string, c: Cfg): string {
  let r = s
  r = r.replace(/rust-wokrers/g, c.slug)
  r = r.replace(/@rust-wokrers\//g, `@${c.scope}/`)
  r = r.replace(/"rust-wokrers"/g, `"${c.slug}"`)
  r = r.replace(/"rust-wokrers-staging"/g, `"${c.slug}-staging"`)
  r = r.replace(/rust-wokrers-doc-index/g, `${c.slug}-doc-index`)
  r = r.replace(/rust-wokrers-staging-doc-index/g, `${c.slug}-staging-doc-index`)
  r = r.replace(/rust-wokrers-media/g, `${c.slug}-media`)
  r = r.replace(/rust-wokrers-staging-media/g, `${c.slug}-staging-media`)
  ...
  r = r.replace(/APP_MODE = "full"/g, `APP_MODE = "${c.appMode}"`)
  return r
}
...
async function main() {
  console.log()
  console.log('  🚀 create-rust-wokrers')
  ...
  try {
    ...
    const appModeInput = await p.ask('실행 모드 (full / api-only)', 'full')
    const appMode = appModeInput === 'api-only' ? 'api-only' : 'full'
    ...
    const cfg: Cfg = { name, slug, scope, appDomain, adminDomain, email, accountId, d1Id: '', d1StagingId: '', imagesHash: '', appMode, ext, agt, vec, aid }
    ...
    fs.writeFileSync(path.join(target, 'rust-workers', '.dev.vars'),
    ...
    const wDir = path.join(target, 'rust-workers')
    ...
    if (await p.confirm(`"${slug}" D1 생성?`)) {
      ...
      else warn('D1 ID를 파싱하지 못했습니다. wrangler.toml에서 직접 설정하세요.')
    }
    ...
    if (await p.confirm('KV 네임스페이스 생성?')) {
      ...
      if (kvMatch) info(`KV ID: ${kvMatch[1]} — wrangler.toml kv_namespaces에 반영하세요`)
    }
    ...
    // wrangler.toml 업데이트
    const wPath = path.join(wDir, 'wrangler.toml')
    ...
    const ml = shSafe(`pnpm --filter @${scope}/rust-workers db:migrate:local`, target)
    ml.ok ? ok('로컬 완료') : warn('로컬 실패 — 나중에 실행하세요')

    if (cfg.d1Id && await p.confirm('원격 D1(prod) 마이그레이션?')) {
      sh(`pnpm --filter @${scope}/rust-workers db:migrate:remote`, target)
      ok('prod 마이그레이션 완료')
    }
    if (cfg.d1StagingId && await p.confirm('원격 D1(staging) 마이그레이션?')) {
      sh(`pnpm --filter @${scope}/rust-workers db:migrate:remote:staging`, target)
    ...
    if (await p.confirm('초기 커밋 생성?')) {
      ...
      sh('git commit -m "Initial scaffold from rust-wokrers boilerplate"', target, true)
      ok('초기 커밋')
    }
